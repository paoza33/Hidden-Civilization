...
public class CubeMovement : MonoBehaviour
{
    ...
    private void SetNewPosition(CubeObject mainCube, string direction)
    {
        if(direction == "Eastward")
        {
            ...
            {
                if (GridManagment.instance.gridScene[mainCube.positionX,i] != null) // on v�rifie si il y a un cube sur le chemin
                {
    ...
    private void FixedUpdate()
    {
        ...
        if (Vector3.Distance(cubeObject.transform.position, newWorldPos) < closeEnough) // les vecteurs ne sont pas egaux � des milliemes pres
        {
            if (!isCube)
            {
